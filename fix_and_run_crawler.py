...
print("Writing fixed crawler...")
print(write_file('/var/www/somekorean/crawler_server/crawler.py', crawler))

# Kill old background process
ssh('pkill -f crawler_server 2>/dev/null')
time_sleep = __import__('time').sleep
time_sleep(1)

# Quick test: 3 cities
print("\nTest: 3 cities...")
print(ssh('cd /var/www/somekorean && python3 crawler_server/crawler.py --city "Los Angeles" --per-city 3 2>&1', timeout=15))

print(ssh("python3 -c \"import pymysql; c=pymysql.connect(host='127.0.0.1',user='somekorean_user',password='SK_DB@2026!secure',database='somekorean'); cur=c.cursor(); cur.execute('SELECT COUNT(*) FROM businesses'); print('Total:', cur.fetchone()[0]); c.close()\""))

# Now run full crawl in background
print("\nRunning full crawl (all 24 cities) in background...")
print(ssh('cd /var/www/somekorean && nohup python3 crawler_server/crawler.py --all --per-city 5 > /tmp/crawl.log 2>&1 &'))

import time; time.sleep(3)
print(ssh('tail -5 /tmp/crawl.log'))
print(ssh('cat /tmp/crawl_status.json 2>/dev/null'))
