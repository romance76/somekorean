...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { to: '/admin/ai-admin',       icon: '🤖', label: 'AI 검색' },
    ]
  },
  {
    label: '비즈니스 / 거래',
...
print(write_file('/var/www/somekorean/resources/js/pages/admin/AdminLayout.vue', new_layout))
print("AdminLayout.vue written")

# Also check if /admin/wallet route exists and remove it from menu (it's not in router)
print("\n=== Checking router for wallet route ===")
raw = ssh('base64 /var/www/somekorean/resources/js/router/index.js')
router_content = base64.b64decode(raw).decode('utf-8')
print("wallet in router:", 'wallet' in router_content)
print("games in router:", 'games' in router_content)
# The menu now uses /admin/payments instead of /admin/wallet, so that's fine
