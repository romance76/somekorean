  ...
  <div class="counting-game" :class="{ shake: shaking, flash: flashing }">
    ...
    <div v-if="gameState === 'playing'" class="playing-screen">
      ...
      <div v-if="answered" class="feedback-msg" :class="lastCorrect ? 'correct-msg' : 'wrong-msg'">
        {{ lastCorrect ? countWords[currentCount] + ' 개! 정답이에요! 🎉' : '다시 세어 봐요 🔄' }}
      </div>
    </div>
...
const countWords = ['', '한', '두', '세', '네', '다섯', '여섯', '일곱', '여덟', '아홉', '열', '열한', '열두', '열세', '열네', '열다섯']
...
function selectAnswer(choice) {
  ...
  if (choice === currentCount.value) {
    ...
    speak(currentObject.value.name + ' ' + countWords[currentCount.value] + ' 개! 정답이에요!')
...
function showResults() {
  ...
  if (correctCount.value >= 8) {
    ...
    speak(countWords[Math.min(starsEarned.value, 15)] + ' 개 맞았어요. 잘했어요!')
  }
