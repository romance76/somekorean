<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      <h2 class="text-lg font-bold text-gray-800">구인구직 관리</h2>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-wrap gap-3">
      ...
      <select v-model="typeFilter" @change="loadItems" class="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white">
        <option value="">전체</option><option value="offer">구인</option><option value="seek">구직</option>
      </select>
    </div>
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3"><span class="text-xs font-semibold px-2 py-0.5 rounded-full" :class="item.type === 'offer' ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-600'">{{ item.type === 'offer' ? '구인' : '구직' }}</span></td>
...
const items = ref([]), search = ref(''), typeFilter = ref(''), page = ref(1), total = ref(0); let dt = null
...
async function loadItems() { try { const { data } = await axios.get('/api/admin/jobs', { params: { search: search.value, type: typeFilter.value, page: page.value } }); items.value = data.data || []; total.value = data.total || data.meta?.total || 0 } catch {} }
async function toggleVis(i) { try { await axios.post(`/api/admin/jobs/${i.id}/toggle`); i.is_hidden = !i.is_hidden } catch {} }
async function del(i) { if (!confirm('삭제할까요?')) return; try { await axios.delete(`/api/admin/jobs/${i.id}`); items.value = items.value.filter(x => x.id !== i.id) } catch {} }
onMounted(loadItems)
</script>
