<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between"><h2 class="text-lg font-bold text-gray-800">동호회 관리</h2><span class="text-sm text-gray-400">총 {{ total }}건</span></div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50 border-b border-gray-200"><tr>
        <th class="text-left px-4 py-3 font-semibold text-gray-600">동호회명</th>
        <th class="text-left px-4 py-3 font-semibold text-gray-600">카테고리</th>
        <th class="text-right px-4 py-3 font-semibold text-gray-600">회원수</th>
        <th class="text-left px-4 py-3 font-semibold text-gray-600">개설자</th>
        ...
        <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50">
          ...
          <td class="px-4 py-3 text-right text-gray-700">{{ item.members_count || 0 }}</td>
          <td class="px-4 py-3 text-gray-600">{{ item.creator?.name || item.user?.name }}</td>
...
async function loadItems() { try { const { data } = await axios.get('/api/admin/clubs', { params: { search: search.value, page: page.value } }); items.value = data.data || []; total.value = data.total || data.meta?.total || 0 } catch {} }
async function toggleVis(i) { try { await axios.post(`/api/admin/clubs/${i.id}/toggle`); i.is_hidden = !i.is_hidden } catch {} }
async function del(i) { if (!confirm('삭제할까요?')) return; try { await axios.delete(`/api/admin/clubs/${i.id}`); items.value = items.value.filter(x => x.id !== i.id) } catch {} }
onMounted(loadItems)
</script>
