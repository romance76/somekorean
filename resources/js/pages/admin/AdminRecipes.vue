<template>
<div>
  <h1 class="text-xl font-black text-gray-800 mb-4">🍳 레시피 관리</h1>
  ...
  <div v-else class="bg-white rounded-xl shadow-sm border overflow-hidden">
    <table class="w-full text-sm">
      <thead class="bg-gray-50 border-b"><tr>
        ...
        <th class="px-3 py-2 text-left text-xs text-gray-500">난이도</th>
        <th class="px-3 py-2 text-left text-xs text-gray-500">좋아요</th>
      ...
      <tbody>
        <tr v-for="item in items" :key="item.id" class="border-b last:border-0 hover:bg-amber-50/30">
          ...
          <td class="px-3 py-2.5 text-xs text-gray-500">{{ item.difficulty }}</td>
          <td class="px-3 py-2.5 text-xs text-gray-500">{{ item.like_count }}</td>
...
async function load(p=1) { loading.value=true; page.value=p; try { const{data}=await axios.get('/api/recipes',{params:{page:p}}); items.value=data.data?.data||data.data||[]; lastPage.value=data.data?.last_page||1 }catch{}; loading.value=false }
async function deleteItem(item) { if(!confirm('삭제?'))return; try { await axios.delete('/api/recipes/'+item.id); items.value=items.value.filter(x=>x.id!==item.id) }catch{} }
onMounted(()=>load())
</script>