<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between"><h2 class="text-lg font-bold text-gray-800">숏츠 관리</h2><span class="text-sm text-gray-400">총 {{ total }}건</span></div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50 border-b border-gray-200"><tr>
        ...
        <th class="text-left px-4 py-3 font-semibold text-gray-600">채널</th>
        <th class="text-right px-4 py-3 font-semibold text-gray-600">조회수</th>
        <th class="text-right px-4 py-3 font-semibold text-gray-600">좋아요</th>
        ...
        <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50">
          ...
          <td class="px-4 py-3 text-gray-600 text-xs">{{ item.channel_name || '-' }}</td>
          <td class="px-4 py-3 text-right text-gray-700">{{ (item.views || 0).toLocaleString() }}</td>
          <td class="px-4 py-3 text-right text-gray-700">{{ item.likes_count || 0 }}</td>
...
async function loadItems() { try { const { data } = await axios.get('/api/admin/shorts', { params: { search: search.value, page: page.value } }); items.value = data.data || []; total.value = data.total || data.meta?.total || 0 } catch {} }
async function toggleVis(i) { try { await axios.post(`/api/admin/shorts/${i.id}/toggle`); i.is_hidden = !i.is_hidden } catch {} }
async function del(i) { if (!confirm('삭제?')) return; try { await axios.delete(`/api/admin/shorts/${i.id}`); items.value = items.value.filter(x => x.id !== i.id) } catch {} }
onMounted(loadItems)
</script>
