<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between"><h2 class="text-lg font-bold text-gray-800">쇼핑 관리</h2><span class="text-sm text-gray-400">총 {{ total }}건</span></div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50 border-b border-gray-200"><tr>
        <th class="text-left px-4 py-3 font-semibold text-gray-600">상품/마트</th>
        <th class="text-left px-4 py-3 font-semibold text-gray-600">유형</th>
        <th class="text-left px-4 py-3 font-semibold text-gray-600">지역</th>
        ...
        <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50">
          <td class="px-4 py-3 font-medium text-gray-800 max-w-xs truncate">{{ item.title || item.name }}</td>
          <td class="px-4 py-3 text-gray-600 text-xs">{{ item.type || 'deal' }}</td>
...
async function loadItems() { try { const { data } = await axios.get('/api/admin/shopping', { params: { search: search.value, page: page.value } }); items.value = data.data || []; total.value = data.total || data.meta?.total || 0 } catch {} }
async function toggleVis(i) { try { await axios.post(`/api/admin/shopping/${i.id}/toggle`); i.is_hidden = !i.is_hidden } catch {} }
async function del(i) { if (!confirm('삭제?')) return; try { await axios.delete(`/api/admin/shopping/${i.id}`); items.value = items.value.filter(x => x.id !== i.id) } catch {} }
onMounted(loadItems)
</script>
