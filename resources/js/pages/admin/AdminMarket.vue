<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between"><h2 class="text-lg font-bold text-gray-800">중고장터 관리</h2><span class="text-sm text-gray-400">총 {{ total }}건</span></div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-wrap gap-3">
      ...
      <select v-model="statusFilter" @change="loadItems" class="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white">
        <option value="">전체</option><option value="selling">판매중</option><option value="reserved">예약</option><option value="sold">판매완료</option>
      </select>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50 border-b border-gray-200"><tr>
        ...
        <th class="text-right px-4 py-3 font-semibold text-gray-600">가격</th>
        ...
        <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50">
          ...
          <td class="px-4 py-3 text-right text-gray-700 font-medium">${{ item.price?.toLocaleString() }}</td>
          <td class="px-4 py-3"><span class="text-xs font-semibold px-2 py-0.5 rounded-full" :class="{'bg-green-50 text-green-600': item.status==='selling', 'bg-yellow-50 text-yellow-600': item.status==='reserved', 'bg-gray-100 text-gray-500': item.status==='sold'}">{{ {selling:'판매중',reserved:'예약',sold:'완료'}[item.status] || item.status }}</span></td>
...
async function loadItems() { try { const { data } = await axios.get('/api/admin/market', { params: { search: search.value, status: statusFilter.value, page: page.value } }); items.value = data.data || []; total.value = data.total || data.meta?.total || 0 } catch {} }
async function toggleVis(i) { try { await axios.post(`/api/admin/market/${i.id}/toggle`); i.is_hidden = !i.is_hidden } catch {} }
async function del(i) { if (!confirm('삭제할까요?')) return; try { await axios.delete(`/api/admin/market/${i.id}`); items.value = items.value.filter(x => x.id !== i.id) } catch {} }
onMounted(loadItems)
</script>
