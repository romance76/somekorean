<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between"><h2 class="text-lg font-bold text-gray-800">Q&A 관리</h2><span class="text-sm text-gray-400">총 {{ total }}건</span></div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-wrap gap-3">
      ...
      <select v-model="statusFilter" @change="loadItems" class="border border-gray-200 rounded-lg px-3 py-2 text-sm bg-white">
        <option value="">전체</option><option value="open">미해결</option><option value="resolved">해결</option>
      </select>
    </div>
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50 border-b border-gray-200"><tr>
        ...
        <th class="text-right px-4 py-3 font-semibold text-gray-600">답변수</th>
        ...
        <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50">
          ...
          <td class="px-4 py-3"><span class="text-xs font-semibold px-2 py-0.5 rounded-full" :class="item.is_resolved ? 'bg-green-50 text-green-600' : 'bg-yellow-50 text-yellow-600'">{{ item.is_resolved ? '해결' : '미해결' }}</span></td>
          <td class="px-4 py-3 text-right text-gray-700">{{ item.answers_count || 0 }}</td>
...
async function loadItems() { try { const { data } = await axios.get('/api/admin/qa', { params: { search: search.value, status: statusFilter.value, page: page.value } }); items.value = data.data || []; total.value = data.total || data.meta?.total || 0 } catch {} }
async function toggleVis(i) { try { await axios.post('/api/admin/qa/' + i.id + '/toggle'); i.is_hidden = !i.is_hidden } catch {} }
async function del(i) { if (!confirm('삭제할까요?')) return; try { await axios.delete('/api/admin/qa/' + i.id); items.value = items.value.filter(x => x.id !== i.id) } catch {} }
onMounted(loadItems)
</script>
