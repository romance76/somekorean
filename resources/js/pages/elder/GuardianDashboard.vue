  ...
  <div class="min-h-screen bg-gray-50 pb-20">
    ...
    <div class="bg-gradient-to-r from-indigo-500 to-indigo-700 text-white px-4 py-6">
      <div class="max-w-3xl mx-auto">
        ...
        <p class="mt-1 text-indigo-200 text-lg">
          피보호자의 안전 상태를 한눈에 확인하세요
        </p>
      </div>
    </div>
    ...
    <div class="max-w-3xl mx-auto px-4 mt-6 space-y-5">
      ...
      <template v-else>
        ...
        <div v-if="sosAlertData" class="bg-red-600 text-white rounded-2xl shadow-lg p-6 text-center animate-pulse">
          <p class="text-4xl mb-2">&#x1F6A8;</p>
          <h2 class="text-2xl font-bold mb-2">긴급 SOS 알림!</h2>
          <p class="text-xl mb-1">{{ sosAlertData.elderName || '어르신' }}님이 SOS를 보냈습니다</p>
          <p v-if="sosAlertData.address" class="text-lg opacity-90 mb-1">위치: {{ sosAlertData.address }}</p>
          <p class="text-base opacity-80 mb-4">{{ sosAlertData.time || formatDateTime(new Date()) }}</p>
          <div class="flex gap-3 justify-center flex-wrap">
            <a
              v-if="sosAlertData.lat && sosAlertData.lng"
              :href="`https://maps.google.com/?q=${sosAlertData.lat},${sosAlertData.lng}`"
              target="_blank"
              class="bg-white text-red-600 font-bold text-lg px-6 py-3 rounded-xl transition hover:bg-red-50"
            >
              지도 보기
            </a>
            <button
              @click="callElder(sosAlertData.elderId ? { id: sosAlertData.elderId, name: sosAlertData.elderName } : selectedElder, 'audio')"
              class="bg-green-500 hover:bg-green-600 text-white font-bold text-lg px-6 py-3 rounded-xl transition"
            >
              전화하기
            </button>
            <a
              href="tel:911"
              class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold text-lg px-6 py-3 rounded-xl transition"
            >
              911 연결
            </a>
            <button
        ...
        <div v-if="!selectedElder && elderList.length > 1" class="bg-white rounded-2xl shadow-md p-6">
        ...
        <!-- No elders connected -->
        <div v-if="!selectedElder && elderList.length === 0 && !loading" class="bg-white rounded-2xl shadow-md p-6 text-center">
          <p class="text-lg text-gray-400 mb-2">연결된 어르신이 없습니다</p>
          <p class="text-base text-gray-400">어르신이 보호자 설정에서 회원님을 등록해야 합니다</p>
        </div>

        <!-- Elder Cards (피보호자 카드 리스트 - always show all when multiple) -->
        <div v-if="elderList.length > 0 && !selectedElder" class="space-y-4">
          <div
            v-for="elder in elderList"
            :key="elder.id"
            class="bg-white rounded-2xl shadow-md p-5 cursor-pointer hover:ring-2 hover:ring-indigo-300 transition"
            @click="selectElder(elder)"
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                <span class="text-3xl">{{ elder.today_checked ? '&#x1F475;' : '&#x1F474;' }}</span>
                <div>
                  <p class="text-xl font-bold text-gray-800">{{ elder.name || elder.nickname }}</p>
                  <p class="text-base text-gray-500">{{ elder.location || '' }}</p>
                </div>
              </div>
              <div class="text-right">
                <p class="text-lg font-bold" :class="elder.today_checked ? 'text-green-600' : getElderStatusColor(elder)">
                  {{ elder.today_checked ? '오늘 체크인' : getElderStatusText(elder) }}
                </p>
                <p class="text-base text-gray-400">{{ getTimeAgo(elder.last_checkin_at) }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <template v-if="selectedElder">
          <!-- Back to list (if multiple) -->
          <button
            v-if="elderList.length > 1"
            @click="selectedElder = null"
            class="text-indigo-500 hover:text-indigo-700 text-lg font-bold flex items-center gap-1"
          >
            &larr; 목록으로
          </button>
          ...
          <div class="bg-white rounded-2xl shadow-md p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="bg-gray-50 rounded-xl p-4 text-center">
                ...
                <p class="text-lg font-bold" :class="statusColor">
              ...
              <!-- Medication -->
              <div class="bg-gray-50 rounded-xl p-4 text-center">
                <p class="text-base text-gray-500 mb-1">복약 현황</p>
                <p class="text-lg font-bold" :class="elderInfo.medication_taken >= elderInfo.medication_total ? 'text-green-600' : 'text-yellow-600'">
                  {{ elderInfo.medication_taken || 0 }}/{{ elderInfo.medication_total || 0 }}
                </p>
              </div>
            </div>
          </div>
          ...
          <div v-if="healthRecords.length" class="bg-white rounded-2xl shadow-md p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">건강 기록 트렌드 (최근 30일)</h2>

            <!-- Simple table view of recent records -->
            <div class="overflow-x-auto">
              <table class="w-full text-base">
                <thead>
                  <tr class="border-b text-gray-500">
                    <th class="text-left py-2 px-2">날짜</th>
                    <th class="text-center py-2 px-2">혈압</th>
                    <th class="text-center py-2 px-2">혈당</th>
                    <th class="text-center py-2 px-2">체중</th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    v-for="record in healthRecords.slice(0, 10)"
                    :key="record.id"
                    class="border-b last:border-b-0"
                  >
                    <td class="py-2 px-2 text-gray-600">{{ formatShortDate(record.recorded_at) }}</td>
                    <td class="py-2 px-2 text-center">
                      <span v-if="record.blood_pressure_systolic" :class="getBPColor(record.blood_pressure_systolic)">
                        {{ record.blood_pressure_systolic }}/{{ record.blood_pressure_diastolic }}
                      </span>
                      <span v-else class="text-gray-300">-</span>
                    </td>
                    <td class="py-2 px-2 text-center">
                      <span v-if="record.blood_sugar" :class="getBSColor(record.blood_sugar)">
                        {{ record.blood_sugar }}
                      </span>
                      <span v-else class="text-gray-300">-</span>
                    </td>
                    <td class="py-2 px-2 text-center">
                      <span v-if="record.weight" class="text-gray-800">
                        {{ record.weight }}kg
                      </span>
                      <span v-else class="text-gray-300">-</span>
                    </td>
          ...
          <div class="bg-white rounded-2xl shadow-md p-6">
            ...
            <div class="grid grid-cols-7 gap-2 text-center text-base mb-2">
            ...
            <div class="grid grid-cols-7 gap-2 text-center">
              ...
              <button
                ...
                class="py-2 rounded-xl text-base transition hover:ring-2 hover:ring-indigo-300"
                :class="{
            ...
            <div v-if="selectedDay" class="mt-4 bg-indigo-50 rounded-xl p-4">
              ...
              <p v-if="selectedDay.status === 'checked'" class="text-green-600 text-lg mt-1">
                {{ selectedDay.time ? selectedDay.time + '에 체크인' : '체크인 완료' }}
              </p>
              <p v-else-if="selectedDay.status === 'missed'" class="text-red-500 text-lg mt-1">
                체크인 미응답
              </p>
              <p v-else class="text-gray-400 text-lg mt-1">아직 도래하지 않은 날짜</p>
            </div>
          </div>
            ...
            <div v-else class="space-y-3">
              ...
              >
                <div class="flex items-start justify-between">
                  <div>
                    <p class="font-bold text-lg" :class="sos.resolved_at ? 'text-gray-600' : 'text-red-700'">
                    ...
                    <a
                      ...
                      target="_blank"
                      class="text-blue-500 text-base hover:underline mt-1 inline-block"
                    >
                      위치 확인 (Google Maps)
                    </a>
                    <p v-if="sos.address" class="text-base text-gray-500 mt-1">{{ sos.address }}</p>
                  </div>
                  <button
                    v-if="!sos.resolved_at"
                    @click="resolveSOS(sos.id)"
                    :disabled="resolvingId === sos.id"
                    class="bg-green-500 hover:bg-green-600 disabled:bg-green-300 text-white font-bold text-base px-5 py-3 rounded-xl"
                  >
          ...
          <div class="bg-white rounded-2xl shadow-md p-6">
            ...
            <div class="space-y-4">
              <div>
                <label class="block text-lg text-gray-600 mb-1">미체크인 알림 (시간)</label>
                <select
                  v-model="guardianSettingsForm.alert_hours"
                  class="w-full border border-gray-300 rounded-xl px-4 py-3 text-lg focus:ring-2 focus:ring-indigo-400"
                >
                  <option :value="6">6시간 후</option>
                  <option :value="12">12시간 후</option>
                  <option :value="24">24시간 후 (기본)</option>
                  <option :value="48">48시간 후</option>
                </select>
              </div>
              <div>
                <label class="block text-lg text-gray-600 mb-1">긴급 연락처</label>
                <input
                  v-model="guardianSettingsForm.emergency_phone"
                  type="tel"
                  placeholder="비상 연락처"
                  class="w-full border border-gray-300 rounded-xl px-4 py-3 text-lg focus:ring-2 focus:ring-indigo-400"
                />
          ...
          <div class="bg-white rounded-2xl shadow-md p-6">
            ...
            <div v-else class="space-y-2">
              <div
                ...
                :class="{
                  'bg-red-50': noti.type === 'sos' || noti.type === 'elder_sos',
                  'bg-yellow-50': noti.type === 'missed' || noti.type === 'elder_checkin_missed',
                  'bg-green-50': noti.type === 'checkin',
                }"
              >
                <span class="text-xl">
                  {{ (noti.type === 'sos' || noti.type === 'elder_sos') ? '&#x1F6A8;' : (noti.type === 'missed' || noti.type === 'elder_checkin_missed') ? '&#x26A0;&#xFE0F;' : '&#x2705;' }}
                </span>
                <div class="flex-1">
                  <p class="text-lg font-medium text-gray-800">{{ noti.body || noti.message }}</p>
                  <p class="text-base text-gray-400">{{ formatDateTime(noti.created_at) }}</p>
                </div>
              </div>
            </div>
          </div>
...
const guardianSettingsForm = ref({
  alert_hours: 24,
  emergency_phone: '',
})
...
function getElderStatusText(elder) {
  if (!elder.last_checkin_at) return '미체크인'
  const hoursAgo = (Date.now() - new Date(elder.last_checkin_at).getTime()) / (1000 * 60 * 60)
  if (hoursAgo < 24) return '안전'
  if (hoursAgo < 48) return '확인필요'
  return '미체크인'
}

function getElderStatusColor(elder) {
  if (!elder.last_checkin_at) return 'text-red-600'
  const hoursAgo = (Date.now() - new Date(elder.last_checkin_at).getTime()) / (1000 * 60 * 60)
  if (hoursAgo < 24) return 'text-green-600'
  if (hoursAgo < 48) return 'text-yellow-600'
  return 'text-red-600'
...
function getBPColor(systolic) {
  if (systolic < 120) return 'text-green-600'
  if (systolic < 140) return 'text-yellow-600'
  return 'text-red-600'
}

function getBSColor(sugar) {
  if (sugar < 100) return 'text-green-600'
  if (sugar < 126) return 'text-yellow-600'
  return 'text-red-600'
}
...
async function selectElder(elder) {
  ...
  try {
    ...
    elderInfo.value = {
      name: elder.name,
      last_checkin_at: data.last_checkin_at || data.settings?.last_checkin_at || elder.last_checkin_at,
      today_checked: data.today_checked ?? false,
      streak: data.streak || 0,
      medication_taken: data.medication_taken || 0,
      medication_total: data.medication_total || 0,
    }
    checkinHistory.value = data.checkin_history || data.checkin_logs || []
    sosHistory.value = data.sos_history || data.sos_logs || []
    healthRecords.value = data.health_records || []
    notifications.value = data.notifications || data.recent_logs || []
  } catch (e) {
