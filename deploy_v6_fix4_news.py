...
sys.stdout.buffer.write(b'\n=== [Fix4] News Sources + Yesterday Summary Widget ===\n')
sys.stdout.buffer.flush()

files = [
    'app/Console/Commands/FetchNews.php',              # 이민/비자 카테고리 + USCIS
    'app/Http/Controllers/API/NewsController.php',     # yesterdaySummary 추가
    'routes/api.php',                                  # /api/news/yesterday-summary 라우트
    'resources/js/pages/Home.vue',                     # 어제의 뉴스 요약 위젯
]
...
sys.stdout.buffer.write(b'\n=== Fetching Fresh News ===\n')
sys.stdout.buffer.flush()
run(f'cd {APP} && php8.2 artisan news:fetch 2>&1 | tail -20', timeout=120)
...
sys.stdout.buffer.write(b'\n=== Testing API ===\n')
sys.stdout.buffer.flush()
run(f'curl -sk "https://somekorean.com/api/news/yesterday-summary" 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(\'date:\', d[\'date\'], \'total:\', d[\'total\'], \'items:\', len(d[\'items\']))"')
