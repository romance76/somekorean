    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-2">
          <button @click="sharePost" class="flex items-center gap-1 text-sm text-gray-500 hover:text-red-600 p-2 sm:px-3 sm:py-1.5 rounded-lg border border-gray-200 hover:border-red-200 transition">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg>
            <span class="hidden sm:inline">공유</span>
          </button>
          <button @click="toggleBookmark" :class="['flex items-center gap-1 text-sm p-2 sm:px-3 sm:py-1.5 rounded-lg border transition', post.is_bookmarked ? 'text-yellow-600 border-yellow-300 bg-yellow-50' : 'text-gray-500 border-gray-200 hover:border-yellow-300']">
      ...
      <div class="flex justify-center gap-3 mb-6">
        <button @click="toggleLike"
          :class="['flex items-center space-x-2 px-3 sm:px-6 py-2 sm:py-2.5 rounded-full border-2 transition font-medium text-sm',
        ...
        <div v-if="authStore.isLoggedIn" class="px-5 py-3 bg-gray-50 border-t border-gray-100">
          <div class="flex flex-col sm:flex-row gap-2">
            ...
            <button @click="submitComment" :disabled="!commentText.trim()"
              class="bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-red-700 disabled:opacity-40 w-full sm:w-auto">등록</button>
          </div>
        </div>
