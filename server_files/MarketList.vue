  ...
  <div class="min-h-screen bg-gray-50 pb-16">
    ...
    <div v-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      <router-link v-for="item in items" :key="item.id" :to="`/market/${item.id}`"
        class="bg-white rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition relative">
        <div class="h-40 bg-gray-100 flex items-center justify-center relative">
          ...
          <!-- 찜 상태 뱃지 -->
          <span v-if="item.status === 'reserved'" class="absolute top-2 left-2 bg-yellow-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">예약중</span>
          <!-- 보증금 뱃지 -->
          <span v-if="item.reservation_points > 0" class="absolute top-2 right-2 bg-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-0.5">
            🔒 {{ item.reservation_points }}P
          </span>
        </div>
        <div class="p-4">
    ...
    <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      <router-link v-for="item in items" :key="item.id" :to="`/market/${item.id}`"
        class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition relative">
