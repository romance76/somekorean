...
class QaController extends Controller {
    ...
    public function index(Request $request) {
        $query = QaPost::with('user:id,name,username', 'category:id,name,key,icon,color')
    ...
    public function show($id) {
        $post = QaPost::with(['user:id,name,username,avatar', 'category:id,name,key,icon,color',
            'answers' => fn($q) => $q->where('is_hidden', false)->with('user:id,name,username,avatar')->orderByDesc('is_best')->orderByDesc('like_count'),
            'bestAnswer.user:id,name,username,avatar',
        ])->findOrFail($id);
    ...
    public function store(Request $request) {
        ...
        return response()->json($post->load('user:id,name,username', 'category'), 201);
    }
    ...
    public function answer(Request $request, $id) {
        ...
        return response()->json(['message' => '답변이 등록되었습니다.', 'answer' => $answer->load('user:id,name,username,avatar')], 201);
    }
